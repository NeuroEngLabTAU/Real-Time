...
from stg._wrapper.constants import STG4002_specification, STG5_specification, TIME_RESOLUTION_in_us
...
class STGX(ABC):
    ...
    def _collect_properties(self):
        ...
        with self.interface() as interface:
            ...
            self._current_resolution_in_ua = (
                    interface.GetCurrentResolutionInNanoAmp(System.UInt32(0))
                    / 1000
            )
            self._current_resolution_in_ma = self._current_resolution_in_ua / 1000
            self._current_range_in_ua = interface.GetCurrentRangeInNanoAmp(
                System.UInt32(0)
            ) / 1000
            self._current_range_in_ma = self._current_range_in_ua / 1000
    ...
    def time_resolution_in_us(self) -> float:
        ...
        if self.name == "STG4002":
            return STG4002_specification[TIME_RESOLUTION_in_us]
        elif self.name == "STG5":
            return STG5_specification[TIME_RESOLUTION_in_us]
        else:
            # raise warning not tested device
            raise NotImplementedError

    @property
    def time_resolution_in_ms(self) -> float:
        "Return the time resolution in ms"
        return self.time_resolution_in_us / 1000
