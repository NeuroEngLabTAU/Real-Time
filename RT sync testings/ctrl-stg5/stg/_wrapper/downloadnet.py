...
from stg._wrapper.constants import CURRENT_RANGE_in_uA, CURRENT_RESOLUTION_in_nA, \
    STG5_specification
...
class STG5(STG4000):
    """
    This class inherits from :class:`~STG4000` and provides additional functionality for the STG5.
    """
    def __init__(self, selected_range: int = 2):
        super().__init__()
        if self.name == 'STG5':
            self._current_range_in_ua =  STG5_specification[CURRENT_RANGE_in_uA][selected_range]
            self._current_range_in_ua = self._current_range_in_ua / 1000
            self._current_resolution_in_ua = STG5_specification[CURRENT_RESOLUTION_in_nA][selected_range] / 1000
